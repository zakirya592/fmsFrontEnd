...
function CreateWorkRequest() {
  ...
  const requestincreas = () => {
    axios
      ...
      .then((res) => {
        ...
        axios
          .put(`/api/workRequestCount_Put/1`, {
            RequestNumber: reqput,
          })
          .then((res) => {
            console.log("Work Request Number put Api", res.data);
            axios
              .get(`/api/workRequestCount_GET_BYID/${reqput}`)
              .then((res) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (res.data.recordsets[0].length === 0) {
          ...
          axios
            .get(`/api/Department_desc_LIST/${Depauto}`)
            .then((res) => {
  ...
  const [dropdownProblemCategoryLIST, setdropdownProblemCategoryLIST] =
    useState([]);

  useEffect(() => {
    // Building_LIST
    axios
      .get(`/api/Building_LIST`)
      .then((res) => {
  ...
  const handleProvinceChange = (e) => {
    ...
    axios
      .get(`/api/Department_desc_LIST/${Deptnale}`)
      .then((res) => {
        setDeptDesc(res.data.recordset[0].DepartmentDesc);
        localStorage.setItem(
          "Departmentname",
          res.data.recordset[0].DepartmentDesc
        );
      })
      .catch((err) => {
        console.log(err);
  ...
  const Workypesdesc = (e) => {
    ...
    axios
      .get(`/api/WorkType_descri_LIST/${Deptnale}`)
      .then((res) => {
        setWorkTypedesc(res.data.recordset[0].WorkTypeDesc);
        localStorage.setItem(
          "WorkTypeDesc",
          res.data.recordset[0].WorkTypeDesc
        );
      })
      .catch((err) => {
        console.log(err);
    ...
    // WorkTrade_LIST
    axios
      .get(`/api/WorkTrade_LIST/${Deptnale}`)
      .then((res) => {
  ...
  const Worktrandedesc = (e) => {
    ...
    axios
      .get(`/api/WorkTrade_descri_LIST/${Deptnale}`)
      .then((res) => {
        setWorkTradedesc(res.data.recordset[0].WorkTradeDesc);
        localStorage.setItem(
          "WorkTradedesc",
          res.data.recordset[0].WorkTradeDesc
        );
      })
      .catch((err) => {
        console.log(err);
  ...
  const AssetDesc = (e) => {
    ...
    axios
      .get(`/api/AssetType_descrip_LIST/${Deptnale}`)
      .then((res) => {
        setAssetTypedesc(res.data.recordset[0].AssetItemDescription);
        axios
          .get(
            `/api/AssetType_model_all_LIST/${res.data.recordset[0].AssetItemDescription}`
          )
          .then((res) => {
            // console.log(res.data);
    ...
    axios
      ...
      .then((res) => {
        if (res.data.recordsets[0].length === 0) {
          ...
          axios
            .get(`/api/WorkType_descri_LIST/${workaout}`)
            .then((res) => {
              // console.log(res.data);
              setWorkTypedesc(res.data.recordset[0].WorkTypeDesc);
            })
  ...
  function handleKeyPressworkrequest(e) {
    if (e.key === "Enter") {
      ...
      axios
        .get(`/api/WorkRequestItems_GET_BYID/${value.RequestNumber}`)
        .then((res) => {
          ...
          axios
            .get(`/api/AssetType_descrip_LIST/${assetdascauto}`)
            .then((res) => {
              setAssetTypedesc(res.data.recordset[0].AssetItemDescription);
              const modellistmode = res.data.recordset[0].AssetItemDescription;
              axios
                .get(`/api/AssetType_model_all_LIST/${modellistmode}`)
                .then((res) => {
                  // console.log(res.data);
                  setManufacturerdesc(res.data.recordset[0].Manufacturer);
                  setAssetCategory(res.data.recordset[0].AssetCategory);
  ...
  const workrequsrpostapi = async () => {
    await axios
      .post(`/api/AddworkRequestsecondPOST`, {
  ...
  const AssetItemTagIDpost = async () => {
    await axios
      .post(`/api/WorkRequestItems_post`, {
        RequestNumber: value.RequestNumber,
        AssetItemTagID: value.AssetItemTagID,
        AssetItemQty: "1",
        AssetItemQtyUsed: "1",
    ...
    axios
      ...
      .then((res) => {
        ...
        const promises = res.data.recordset.map((item) => {
          ...
          return axios
            .get(`/api/tblAssetsMaster_GET_BYID/${itid}`)
            .then((res) => {
        ...
        Promise.all(promises).then((results) => {
          ...
          results.forEach((itemRecords, index) => {
            console.log(
              `Records for ${AssetItemDescriptionsss[index]}:`,
              itemRecords.data[0]
            );
            // setgetdata(results);
            const recordsWithDescriptions = AssetItemDescriptionsss.map(
              (description, index) => ({
  ...
  const columns = [
    ...
    {
      field: "AssetItemDescription",
      headerName: "ASSET ITEM DESCRIPTION",
      width: 220,
    },
    ...
    swalWithBootstrapButtons
      ...
      .then((result) => {
        if (result.isConfirmed) {
          axios
            .delete(`/api/assetworkrequest_DELETE_BYID/${ASQS}`)
            .then((res) => {
              getapi();
              ...
              <div className="py-3">
                ...
                <div className="d-flex justify-content-between my-auto">
                  ...
                  <div className="d-flex">
                    ...
                    <button
                      type="button"
                      className="btn btn-outline-primary mx-1 color2 btnwork btnworkactive"
                      onClick={allCreateapi}>
                      {" "}
                    ...
                    <CSVLink
                      data={getdata}
                      type="button"
                      className="btn btn-outline-primary color2">
                      {" "}
                      <img
                        src={excel}
                        alt="export"
                ...
                <form>
                    ...
                    <div className="col-sm-12 col-md-3 col-lg-3 col-xl-3 ">
                      <div className="emailsection position-relative d-grid my-2">
                        ...
                        <input
                          ...
                          required></input>
                        <p className="position-absolute text-end serachicon">
                          <SearchOutlined
                    ...
                    <div className="col-sm-12 col-md-3 col-lg-3 col-xl-3 ">
                      <div className="emailsection position-relative d-grid my-2">
                        ...
                        <select
                          ...
                          <option className="inputsectiondropdpwn" value="Open">
                            Open
                          </option>
                          {RequestStatusLIST &&
                            RequestStatusLIST.map((itme, index) => {
                              return (
                                <option
                                  key={index}
                                  value={itme.RequestStatusCode}>
                                  {itme.RequestStatusCode}
                                </option>
                              );
                            })}
                        </select>
                    ...
                    <div className="col-sm-12 col-md-3 col-lg-3 col-xl-3 ">
                      <div className="emailsection position-relative d-grid my-2">
                        ...
                        <select
                          ...
                          <option className="inputsectiondropdpwn">
                            Select Dept Code
                          </option>
                          {dropdownDepartmentLIST &&
                            dropdownDepartmentLIST.map((itme, index) => {
                              return (
                                <option key={index} value={itme.DepartmentCode}>
                                  {itme.DepartmentCode}
                                </option>
                              );
                            })}
                      ...
                      <div className="emailsection position-relative d-grid my-2">
                        ...
                        <select
                          ...
                          {dropdownBuildingLIST &&
                            dropdownBuildingLIST.map((itme, index) => {
                              return (
                                <option key={index} value={itme.BuildingCode}>
                                  {itme.BuildingCode}
                                </option>
                              );
                            })}
                    ...
                    <div className="col-sm-12 col-md-2 col-lg-2 col-xl-2 ">
                      <div className="emailsection position-relative d-grid my-2">
                        ...
                        <select
                          ...
                          {dropdownLocation &&
                            dropdownLocation.map((itme, index) => {
                              return (
                                <option key={index} value={itme.LocationCode}>
                                  {itme.LocationCode}
                                </option>
                              );
                            })}
                    ...
                    <div className="col-sm-12 col-md-3 col-lg-3 col-xl-3 ">
                      <div className="emailsection position-relative d-grid my-2">
                        ...
                        <select
                          ...
                          {dropdownworktypesLIST &&
                            dropdownworktypesLIST.map((itme, index) => {
                              return (
                                <option key={index} value={itme.WorkTypeCode}>
                                  {itme.WorkTypeCode}
                                </option>
                              );
                            })}
                    ...
                    <div className="col-sm-12 col-md-3 col-lg-3 col-xl-3 ">
                      <div className="emailsection position-relative d-grid my-2">
                        ...
                        <select
                          ...
                          {dropdownWorkPriorityLIST &&
                            dropdownWorkPriorityLIST.map((itme, index) => {
                              return (
                                <option
                                  key={index}
                                  value={itme.WorkPriorityCode}>
                                  {itme.WorkPriorityCode}
                                </option>
                              );
                            })}
                        </select>
                  ...
                  <div className="row mx-auto formsection">
                    <div className="col-sm-12 col-md-3 col-lg-3 col-xl-3 ">
                      <div className="emailsection position-relative d-grid my-2">
                        ...
                        <select
                          ...
                          <option className="inputsectiondropdpwn">
                            {localStorage.getItem("WorkTradeCode") ||
                              "Select Work Trade"}
                          </option>
                          {dropdownWorkTradeLIST &&
                            dropdownWorkTradeLIST.map((itme, index) => {
                              return (
                                <option key={index} value={itme.WorkTradeCode}>
                                  {itme.WorkTradeCode}
                                </option>
                              );
                            })}
                    ...
                    <div className="col-sm-3 my-auto col-md-3 col-lg-3 col-xl-3 ">
                      <button
                        type="button"
                        className="btn btn-outline-primary mx-1 color2 btnwork mt-3 btnworkactive"
                        onClick={Assetcodebtn}>
                        {" "}
                        <AddCircleOutlineIcon className="me-1" />
                        Asset Code
                      </button>
