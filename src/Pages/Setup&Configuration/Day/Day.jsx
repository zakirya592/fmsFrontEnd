...
function Day() {
    ...
    const columns = [
        ...
        { field: 'DaysCode', headerName: 'DAYS CODE', width: 150 },
        { field: 'DaysSeq', headerName: 'Days Seq', width: 150 },
        { field: 'DaysDesc', headerName: 'DESCRIPTION', width: 270 },
        {
            field: 'action',
            headerName: 'ACTION',
            width: 170,
            renderCell: (params) => (
                <div>
                    <button type="button" className="btn  mx-1 color2 btnwork" onClick={() => updata(params.row.DaysCode)}>
                    ...
                    <button type="button" className="btn  mx-1 color2 btnwork" onClick={() => Deletedapi(params.row.DaysCode)}>
                        <DeleteOutlineIcon />
                    </button>
                </div>
            ),
    ...
    const filteredData = getdata && getdata.map((row, indes) => ({
        ...
        id: indes + 1,
        // SEQ:row.EmployeeID,
        DaysCode: row.DaysCode,
        DaysDesc: row.DaysDesc,
        DaysSeq: row.DaysSeq
    ...
    function updata(DaysCode) {
        ...
        axios.get(`/api/Days_GET_BYID/${DaysCode}`, {
            ...
            .then((res) => {
                ...
                setDaysSeq(res.data.recordset[0].DaysSeq)
                setDaysDesc(res.data.recordset[0].DaysDesc)
                setItemCode(DaysCode); // Store the WorkTypeCode in state
            })
            .catch((err) => {
                console.log(err);
    ...
    const postapi = (e) => {
        ...
        axios.put(`/api/Days_Put/${itemCode}`, {
            DaysSeq: DaysSeq,
            DaysDesc: DaysDesc,
        },)
            .then((res) => {
                    ...
                    <div className="topermaringpage  container">
                        <div className="py-3">
                            <div className="d-flex justify-content-between my-auto">
                                <p className="color1 workitoppro my-auto">DAYS MAINTENANCE
                    ...
                    <div class="modal-dialog bgupdata" style={{ borderRadius: '10px', border: '4px solid #1E3B8B' }}>
                        <div class="modal-content bgupdata">
                            <div class="modal-header">
                                <h5 class="modal-title" id="staticBackdropLabel">Updata Days </h5>
