...
import NewLoaction from '../../../Component/AllRounter/setup configuration/Loaction/NewLoaction';
...
function Loactioncode() {
    const ref = useRef(null)
    const [itemCode, setItemCode] = useState(null);
    const [eLocationDesc, seteLocationDesc] = useState()
    const [open, setOpen] = useState(false)
    const columns = [
        { field: 'id', headerName: 'SEQ.', width: 150 },
        { field: 'LocationCode', headerName: 'LOCATION CODE', width: 270 },
        { field: 'LocationDesc', headerName: 'DESCRIPTION', width: 270 },
        {
            field: 'action',
            headerName: 'ACTION',
            width: 170,
            renderCell: (params) => (
                <div>
                    <button type="button" className="btn  mx-1 color2 btnwork" onClick={() => updata(params.row.LocationCode)}>
                    ...
                    <button type="button" className="btn  mx-1 color2 btnwork" onClick={() => Deletedapi(params.row.LocationCode)}>
                        <DeleteOutlineIcon />
                    </button>
                </div>
            ),
    ...
    function updata(LocationCode) {
        ...
        axios.get(`/api/Location_GET_BYID/${LocationCode}`, {
            ...
            .then((res) => {
                ...
                seteLocationDesc(res.data.recordset[0].LocationDesc)
                setItemCode(LocationCode); // Store the WorkTypeCode in state
            })
            .catch((err) => {
                console.log(err);
    ...
    const postapi = (e) => {
        ...
        axios.put(`/api/Location_Put/${itemCode}`, {
            LocationDesc: eLocationDesc,
        },)
            .then((res) => {
            ...
            reader.onload = (e) => {
                ...
                json.forEach((item) => {
                    axios.post(`/api/Location_post`, {
                        LocationCode: item.LocationCode, // Adjust property names as needed
                        ...
                        .catch((err) => {
                            ...
                            Swal.fire(
                                'Error!',
                                `Some Location Code already exist`,
                                'error'
                            )
                            // Handle errors
                        });
                    ...
                    <div className="topermaringpage  container">
                        <div className="py-3">
                            <div className="d-flex justify-content-between my-auto">
                                <p className="color1 workitoppro my-auto">LOCATION MAINTENANCE
                            ...
                            <form onSubmit={postapi}>
                                <div className='emailsection position-relative d-grid my-1'>
                                    ...
                                    <input
                                        ...
                                        value={eLocationDesc}
                                        onChange={e => {
                                            seteLocationDesc(e.target.value)
                                        }}
