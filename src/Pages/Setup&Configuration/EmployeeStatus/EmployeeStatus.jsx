...
function EmployeeStatus() {
    ...
    const [EmployeeStatusDesc, setEmployeeStatusDesc] = useState()
    ...
    const Deletedapi = (EmployeeStatusCode) => {
        console.log(EmployeeStatusCode);
        ...
        swalWithBootstrapButtons.fire({
            title: 'Are you sure?',
            text: `You want to delete this ${EmployeeStatusCode} Employee Status`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                swalWithBootstrapButtons.fire(
                    'Deleted!',
                    `Employee Status ${EmployeeStatusCode} has been Deleted`,
                    'success'
                )
            }
        })
        ...
        { field: 'description', headerName: 'DESCRIPTION', width: 350 },
        {
            ...
            renderCell: (params) => (
                <div>
                    ...
                    <button type="button" className="btn  mx-1 color2 btnwork" onClick={() => Deletedapi(params.row.EmployeeStatusCode)}>
                        <DeleteOutlineIcon />
                    </button>
                </div>
            ),
    ...
    const filteredData = getdata && getdata.map((row, indes) => ({
        ...
        id: indes + 1,
        // SEQ:row.EmployeeID,
        WORKSTATUS: row.EmployeeStatusCode,
        description: row.EmployeeStatusDesc

    }))

    // Updata section
    // GEt by id Api
    function updata(EmployeeStatusCode) {
        console.log(EmployeeStatusCode);
        ref.current.click()
        // get api
        axios.get(`/api/EmployeeStatus_GET_BYID/${EmployeeStatusCode}`, {
            ...
            .then((res) => {
                ...
                setEmployeeStatusDesc(res.data.recordset[0].EmployeeStatusDesc)

                setItemCode(EmployeeStatusCode); // Store the WorkTypeCode in state
            })
            .catch((err) => {
                console.log(err);
    ...
    const postapi = (e) => {
        ...
        axios.put(`/api/EmployeeStatus_Put/${itemCode}`, {
            ...
            .then((res) => {
                ...
                Swal.fire(
                    'Update!',
                    `Employee Status ${itemCode} has been updated`,
                    'success'
                ).then(() => {
                    handleClose();
                });
                    ...
                    <div className="topermaringpage  container">
                        <div className="py-3">
                            <div className="d-flex justify-content-between my-auto">
                                <p className="color1 workitoppro my-auto">EMPLOYEE STATUS MAINTENANCE
                        ...
                        <div class="modal-body">
                            <form onSubmit={postapi}>
                                ...
                                <div className='emailsection position-relative d-grid my-1'>
                                    <label htmlFor='EmployeeStatusDesc' className='lablesection color3 text-start mb-1'>
                                        WorkStatus Desc<span className='star'>*</span>
                                    </label>

                                    <input
                                        types='text'
                                        id='EmployeeStatusDesc'
                                        value={EmployeeStatusDesc}
                                        onChange={e => {
                                            setEmployeeStatusDesc(e.target.value)
                                        }}
