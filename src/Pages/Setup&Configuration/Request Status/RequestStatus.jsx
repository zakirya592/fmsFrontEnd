...
import Newreaueststatus from '../../../Component/AllRounter/setup configuration/Request Status/Newreaueststatus';
...
function RequestStatus() {
    ...
    const [eRequestStatusDesc, seteRequestStatusDesc] = useState()
    const [open, setOpen] = useState(false)
    const columns = [
        { field: 'id', headerName: 'SEQ.', width: 150 },
        { field: 'RequestStatusCode', headerName: 'REQUEST STATUS CODE', width: 270 },
        { field: 'RequestStatusDesc', headerName: 'DESCRIPTION', width: 270 },
        {
            field: 'action',
            headerName: 'ACTION',
            width: 170,
            renderCell: (params) => (
                <div>
                    <button type="button" className="btn  mx-1 color2 btnwork" onClick={() => updata(params.row.RequestStatusCode)}>
                    ...
                    <button type="button" className="btn  mx-1 color2 btnwork" onClick={() => Deletedapi(params.row.RequestStatusCode)}>
                        <DeleteOutlineIcon />
                    </button>
                </div>
            ),
    ...
    const filteredData = getdata && getdata.map((row, indes) => ({
        ...
        id: indes + 1,
        // SEQ:row.EmployeeID,
        RequestStatusCode: row.RequestStatusCode,
        RequestStatusDesc: row.RequestStatusDesc

    }))

     // Updata section
    // GEt by id Api
    function updata(RequestStatusCode) {
        console.log(RequestStatusCode);
        ref.current.click()
        // get api
        axios.get(`/api/RequestStatus_GET_BYID/${RequestStatusCode}`, {
            ...
            .then((res) => {
                ...
                seteRequestStatusDesc(res.data.recordset[0].RequestStatusDesc)
                setItemCode(RequestStatusCode); // Store the WorkTypeCode in state
            })
            .catch((err) => {
                console.log(err);
    ...
    const postapi = (e) => {
        ...
        axios.put(`/api/RequestStatus_Put/${itemCode}`, {
            RequestStatusDesc: eRequestStatusDesc,
        },)
            .then((res) => {
            ...
            reader.onload = (e) => {
                ...
                json.forEach((item) => {
                    ...
                    axios.post(`/api/RequestStatus_post`, {
                        RequestStatusCode: item.RequestStatusCode, // Adjust property names as needed
                        RequestStatusDesc: item.RequestStatusDesc,
                    })
                        .then((res) => {
                            console.log('Add', res.data);
                            // Handle success
                    ...
                    <div className="topermaringpage  container">
                        <div className="py-3">
                            <div className="d-flex justify-content-between my-auto">
                                <p className="color1 workitoppro my-auto">REQUEST STATUS MAINTENANCE
                            ...
                            <form onSubmit={postapi}>
                                <div className='emailsection position-relative d-grid my-1'>
                                    ...
                                    <input
                                        types='text'
                                        id='DepartmentDesc'
                                        value={eRequestStatusDesc}
                                        onChange={e => {
                                            seteRequestStatusDesc(e.target.value)
                                        }}
                                        className='rounded inputsection py-2 borderfo'
                                        placeholder='RequestStatus Desc'
                                        required
                                    ></input>
                                </div>
